...
//     // Save the PDF
//     doc.save("varnix-total-billing-statement.pdf");
...
const VarnixInvoice = () => {
  ...
  // Helper for Indian Currency Formatting (Screen)
  ...
  // Helper for PDF Number Formatting (No Symbol, just commas)
  ...
  const generatePDF = () => {
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(22);
    doc.text("Varnix Total Billing Statement", pageWidth / 2, 20, { align: "center" });

    // --- Date ---
    doc.setFont("helvetica", "normal");
    doc.setFontSize(11);
    doc.setTextColor(100);
    doc.text(`Updated on ${format(new Date(), "MMMM d, yyyy")}`, pageWidth / 2, 30, { align: "center" });
    doc.setTextColor(0); // Reset color

    // --- Section 1: Projects ---
    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.text("All Projects & Cost", 14, 45);
    ...
    autoTable(doc, {
      ...
      body: varnixProjects.map((p) => [
        p.project_name,
        `Rs. ${formatNumber(p.development_cost)}`, // Using "Rs." to avoid glyph errors
        `Rs. ${formatNumber(p.additional_cost)}`,
        `Rs. ${formatNumber(p.cost)}`,
        p.status.charAt(0).toUpperCase() + p.status.slice(1),
      ]),
      theme: 'grid',
      headStyles: { 
    ...
    doc.roundedRect(14, secondTableEndY, pageWidth - 28, 45, 3, 3, 'F');
    ...
    doc.text(`Total Projects Value:`, 20, secondTableEndY + 12);
    doc.text(`Rs. ${formatNumber(totalProjectValue)}`, pageWidth - 20, secondTableEndY + 12, { align: "right" });

    // Received
    doc.setTextColor(34, 197, 94); // Green
    doc.text(`Total Received Amount:`, 20, secondTableEndY + 22);
    doc.text(`Rs. ${formatNumber(totalAmountReceived)}`, pageWidth - 20, secondTableEndY + 22, { align: "right" });

    // Divider
    doc.setDrawColor(220, 220, 220);
    doc.line(20, secondTableEndY + 28, pageWidth - 20, secondTableEndY + 28);

    // Remaining
    doc.setFontSize(14);
    doc.setTextColor(239, 68, 68); // Red
    doc.text(`Total Remaining Amount to Pay:`, 20, secondTableEndY + 38);
    doc.text(`Rs. ${formatNumber(totalAmountRemaining)}`, pageWidth - 20, secondTableEndY + 38, { align: "right" });

    doc.save("Varnix_Billing_Statement.pdf");
  ...
  return (
    <DashboardLayout title="Varnix Invoice" description="Single-client consolidated details">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5 mb-8">
        ...
        <StatCard
          title="Amount Remaining"
          value={formatINR(totalAmountRemaining)}
          icon={IndianRupee}
        />
      </div>
              ...
              <TableBody>
                {varnixProjects.length === 0 ? (
                  ...
                  varnixProjects.map((project) => (
                    <TableRow key={project.id}>
                      <TableCell className="font-medium">{project.project_name}</TableCell>
                      <TableCell>{formatINR(project.development_cost)}</TableCell>
                      <TableCell>{formatINR(project.additional_cost)}</TableCell>
                      <TableCell className="font-semibold">{formatINR(project.cost)}</TableCell>
                      <TableCell>
                        <span className={`text-xs px-2 py-1 rounded-full capitalize ${
                          project.status === "completed" ? "bg-success/10 text-success" :
                          project.status === "ongoing" ? "bg-primary/10 text-primary" :
      ...
      <div className="mt-8 dashboard-section p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <div className="p-4 rounded-lg bg-secondary">
            <p className="text-sm text-muted-foreground">Total Project Value</p>
            <p className="text-2xl font-bold">{formatINR(totalProjectValue)}</p>
          </div>
          <div className="p-4 rounded-lg bg-success/10">
            <p className="text-sm text-muted-foreground">Total Received</p>
            <p className="text-2xl font-bold text-success">{formatINR(totalAmountReceived)}</p>
          </div>
          <div className="p-4 rounded-lg bg-warning/10">
            <p className="text-sm text-muted-foreground">Total Remaining</p>
            <p className="text-2xl font-bold text-warning">{formatINR(totalAmountRemaining)}</p>
          </div>
        </div>
      </div>
